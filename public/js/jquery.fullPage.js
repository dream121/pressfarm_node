	...
	$.fn.fullpage = function(options) {
		...
		$.fn.fullpage.setAutoScrolling = function(value){
			...
			if(options.autoScrolling && !options.scrollBar){
				...
				$('html, body,.bg-black').scrollTop(element.position().top);
			}
		...
		*/
		...
		$('.fp-section').each(function(index){
			...
			if(options.fixedElements && options.css3){
				$(options.fixedElements).appendTo('.bg-black');
			}
			...
			if(options.menu && options.css3 && $(options.menu).closest('.fullpage-wrapper').length){
				$(options.menu).appendTo('.bg-black');
			}
			...
			if(destiny.length){
				...
				if(!options.animateAnchor && section.length){
					...
					if(options.autoScrolling){
						...
						//scrolling the page to the section with no animation
						$('html, body,.bg-black').scrollTop(section.position().top);
					}
		...
		*/
		function addVerticalNavigation(){
			$('.bg-black').append('<div id="fp-nav"><ul></ul></div>');
			nav = $('#fp-nav');
		...
		*/
		function getScrollSettings(v){
			...
			if(options.autoScrolling && !options.scrollBar){
				...
				scroll.element = 'html, body,.bg-black';
			}
		...
		*/
		...
		function resizeMe(displayHeight, displayWidth) {
			...
			if (displayHeight < preferredHeight || displayWidth < preferredWidth) {
				...
				$("bg-black").css("font-size", newFontSize + '%');
			} else {
				$("bg-black").css("font-size", '100%');
			}
		}
		...
		*/
		function setBodyClass(text){
			...
			$(".bg-black")[0].className = $(".bg-black")[0].className.replace(/\b\s?fp-viewing-[^\s]+\b/g, '');

			//adding the current anchor
			$(".bg-black").addClass("fp-viewing-" + text);
		}
		...
		*/
		function destroyStructure(){
			...
			$('html, body,.bg-black').scrollTop(0);
		}
